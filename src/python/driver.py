...
def zipLambda(fname, zipname):
    """
    Libraries and code zip packaging
    """
...
def write_to_s3(bucket, key, data, metadata):
    """
    Save file name, json(data) in S3 Bucket
    """
...
def write_job_config(job_id, job_bucket, n_mappers, r_func, r_handler):
    """
    Store information about the running job locally as json
    """
    fname = "jobinfo.json"
...
# 1.Save all key-values ​​in the configuration file driverconfig) json file for Driver Job
...
# The number of batches finally obtained is determined by the mapper.
...
# Packaging and packing the lambda_handler code for each mapper and reducer and coordinator.
...
# Register the Mapper with the Lambda Function.
...
# Register the Reducer with the Lambda Function.
...
# Register the Coordinator with the Lambda Function.
...
# create s3 as the event source for coordinator
...
xray_recorder.current_subsegment().put_metadata(
    "Job data: ", data, "Write job data to S3")
...
while mappers_executed < n_mappers:
    ...
    mappers_executed += nm
    xray_recorder.current_subsegment().put_metadata(
        "Mapper lambdas executed: ", mappers_executed, "Invoke mappers")
